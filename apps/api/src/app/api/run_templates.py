...
router = APIRouter(prefix="/workspaces/{workspace_id}/run-templates", tags=["run-templates"])
...
class RunTemplateOut(BaseModel):
    ...
    # library-aware metadata (same spirit as pipelines)
...
# ---- Library helpers (compatible with your Workspace.template_admin_json approach) ----
...
def _resolve_run_template_or_404(
    db: Session, *, consumer_ws: Workspace, template_id: uuid.UUID
) -> Tuple[models.RunTemplate, bool, Optional[str]]:
    t = db.get(models.RunTemplate, template_id)
    if not t:
        raise HTTPException(status_code=404, detail="Run template not found")

    allowed_ids, label_map = _allowed_template_workspace_ids(consumer_ws)
    if t.workspace_id not in allowed_ids:
        # hide existence if not allowed
        raise HTTPException(status_code=404, detail="Run template not found")
...
def list_run_templates(
    ...
    rows = (
        db.execute(
            select(models.RunTemplate)
            .where(models.RunTemplate.workspace_id.in_(allowed_ids))
            .order_by(models.RunTemplate.updated_at.desc(), models.RunTemplate.created_at.desc())
        )
        .scalars()
        .all()
    )

    out: List[RunTemplateOut] = []
    for t in rows:
        is_lib = str(t.workspace_id) != str(consumer_ws.id)
        lib_label = label_map.get(str(t.workspace_id)) if is_lib else None
        out.append(
...
def get_run_template(
    ...
    consumer_ws, _role = require_workspace_role_min(str(workspace_id), "member", db, user)
    t, is_lib, lib_label = _resolve_run_template_or_404(db, consumer_ws=consumer_ws, template_id=template_id)
...
@router.post("", response_model=RunTemplateOut)
def create_run_template(
    workspace_id: uuid.UUID,
    payload: RunTemplateCreate,
    db: Session = Depends(get_db),
    user: models.User = Depends(require_user),
) -> RunTemplateOut:
    consumer_ws, _role = require_workspace_role_min(str(workspace_id), "member", db, user)

    t = models.RunTemplate(
    ...
    return RunTemplateOut.model_validate(t)
...
def update_run_template(
    workspace_id: uuid.UUID,
    template_id: uuid.UUID,
    payload: RunTemplateUpdate,
    db: Session = Depends(get_db),
    user: models.User = Depends(require_user),
) -> RunTemplateOut:
    consumer_ws, _role = require_workspace_role_min(str(workspace_id), "member", db, user)
    t, is_lib, _lib_label = _resolve_run_template_or_404(db, consumer_ws=consumer_ws, template_id=template_id)
